...
export async function POST(
  ...
  const [neg] = await sql`SELECT address, role FROM negotiations WHERE id = ${negotiationId}`;
  ...
  const prompt = buildFirstContactPrompt(neg.address, neg.role, offerAmount, notes);
